...
- `assemble_veteran_auction_pool` / `open_scheduled_auction` / `slide_unbid_auctions_down_a_tier` (`assemble_veteran_pool.rs`) — builds `auction_schedule` + tier assignments from `eligibility::build_veteran_auction_pool`, opens rows idempotently on their release date, slides unbid auctions one tier per day. **Not yet reachable in production:** nothing calls `assemble_veteran_auction_pool` (fbkl-rust-j62) and nothing writes the ranked list or the min-bid tiers it reads (fbkl-rust-7qi), so no `auction_schedule` row is ever written. A settled schedule row also still re-fires every tick (fbkl-rust-esp).
