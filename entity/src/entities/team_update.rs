...
impl TeamUpdateData {
    pub fn create_roster_update_from_contracts(
        contracts: &[contract::Model],
        contract_update_type: ContractUpdateType,
    ) -> Result<Self> {
        let team_update_data_contracts = contracts
            .iter()
            .map(|contract| ContractUpdate {
                contract_id: contract.id,
                update_type: contract_update_type,
            })
            .collect();
        let team_update_data = TeamUpdateData::Roster(team_update_data_contracts);
        Ok(team_update_data)
    }
