...
impl RelatedPlayer {
    pub fn get_name(&self) -> &str {
        match self {
            Self::LeaguePlayer(lp) => &lp.name,
            Self::Player(p) => &p.name,
        }
    }

    pub const fn get_is_rdi_eligible(&self) -> bool {
        match self {
            Self::LeaguePlayer(lp) => lp.is_rdi_eligible,
            Self::Player(p) => p.is_rdi_eligible,
        }
    }
}
...
fn validate_contract_years_by_type(model: &ActiveModel) -> Result<(), DbErr> {
        ...
        .find(|(contract_type, _years_allowed)| contract_type == &model.kind.as_ref())
    {
        ...
        Some((_contract_type, valid_years_for_contract_type)) => {
            if valid_years_for_contract_type.contains(model.year_number.as_ref()) {
                Ok(())
            } else {
                Err(DbErr::Custom(format!(
