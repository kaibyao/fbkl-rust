 ...
 * (`lib/contract.utils.ts`), keyed by the backend `ContractKind` enum, and `<ContractChip>` renders
 * them. This table holds only the abbreviations that have no other home, and it grows one entry per
 * screen that prints a new one.
 */
export const TERMS = {
